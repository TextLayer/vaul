...
def test_schema_generation_consistency():
    ...
    @tool_call(retry=True, raise_for_exception=True, concurrent=True, max_timeout=30, max_backoff=60)
...
def test_edge_case_parameter_handling():
    ...
    @tool_call(max_timeout=1, max_backoff=2, concurrent=True)
...
def test_timeout_parameter_validation():
    ...
    @tool_call(retry=True, raise_for_exception=True, max_timeout=5, max_backoff=10)
    def valid_timeout_function(x: int) -> int:
        return x

    is_equal(valid_timeout_function._max_timeout, 5)
    is_equal(valid_timeout_function._max_backoff, 10)

    @tool_call(retry=True, raise_for_exception=True)
    def default_timeout_function(x: int) -> int:
        return x

    is_equal(default_timeout_function._max_timeout, 60)
    is_equal(default_timeout_function._max_backoff, 120)
