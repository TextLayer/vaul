...
class TestConcurrencyAndThreadSafety:
    ...
    def test_concurrent_tool_execution(self):
        ...
        calc_results = [r for r in all_results if r == 328350]  # sum of squares 0-99
        
        assert len(add_results) == 50  # 5 threads * 10 iterations
    ...
    def test_thread_safe_toolkit_operations(self):
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for i in range(5):
                futures.append(
                    executor.submit(add_tools_worker, i * 10, 10)
                )
        ...
        assert len(toolkit.tool_names) == len(set(toolkit.tool_names))  # No duplicates
    ...
    def test_concurrent_tool_removal(self):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            futures = [
                executor.submit(remove_tools_worker, batch)
                for batch in tool_batches
            ]
            
            total_removed = sum(future.result() for future in as_completed(futures))
        
        assert total_removed >= 15, f"Expected at least 15 removals, got {total_removed}"
        assert len(toolkit) <= 5, f"Expected at most 5 tools remaining, got {len(toolkit)}"
    ...
    def test_thread_isolation(self):
        ...
        assert len(thread_ids) >= 1, f"Should have at least 1 thread ID, got {len(thread_ids)}"
        assert len(thread_ids) <= 5, f"Should have at most 5 different thread IDs, got {len(thread_ids)}"
        ...
        for thread_id, executions in by_thread.items():
            assert len(executions) >= 1, f"Thread {thread_id} should have at least 1 execution"
    ...
    def test_deadlock_prevention(self):
        ...
        with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [
                executor.submit(complex_operations, i) for i in range(8)
            ]
        ...
        assert total_time < 10, f"Operations took too long, possible deadlock: {total_time}s"
        assert len(all_results) == 80  # 8 threads * 10 operations each
    ...
    def test_resource_contention(self):
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            futures = [
                executor.submit(high_contention_worker, i) for i in range(20)
            ]
        ...
        assert len(all_results) == 100  # 20 workers * 5 operations each
